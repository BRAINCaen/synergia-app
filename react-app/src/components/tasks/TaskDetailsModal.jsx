...
const TaskDetailModal = ({ 
  ...
  useEffect(() => {
    ...
    console.log('📡 [TASK_MODAL] Configuration listener temps réel pour tâche:', task.id);
    ...
    const unsubscribe = collaborationService.subscribeToComments(
      ...
      task.id,
      (updatedComments) => {
        console.log('📡 [TASK_MODAL] Commentaires mis à jour en temps réel:', updatedComments.length);
    ...
    return () => {
      if (unsubscribe) {
        console.log('🛑 [TASK_MODAL] Nettoyage listener pour tâche:', task.id);
        unsubscribe();
      }
    };
  }, [task?.id]);
