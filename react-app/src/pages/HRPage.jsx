...
import { GlassCard, StatCard, SignatureCanvas, EmployeeCard, EmployeesTab, LeavesTab } from '../components/hr';
...
const PayrollTab = ({ employees, timesheets, leaves, companyName, onRefresh, currentUser, isAdmin }) => {
  ...
  // ✍️ États pour la validation par signature
  ...
  useEffect(() => {
    const loadValidationPeriods = async () => {
      try {
        ...
        // Charger la validation pour la période sélectionnée
        ...
        if (validationDoc.exists()) {
          ...
          // Créer une période de validation vide
  ...
  // ✍️ Signer les pointages (Employé)
  const handleSignTimesheet = async (signatureData) => {
    ...
    try {
      ...
      // Mettre à jour la signature de l'employé
  ...
  // 📤 Envoyer à la paie (Gestionnaire)
  ...
  // ✏️ Modifier un pointage (Gestionnaire)
  const handleEditTimeEntry = async (entry, newData) => {
    try {
      ...
      await updateDoc(entryRef, {
        ...
        originalData: entry // Garder l'historique
      });
  ...
  // ➕ Ajouter un pointage manquant (Gestionnaire)
  ...
  return (
    ...
    >
      {/* ==========================================
          ✍️ SECTION VALIDATION PAR SIGNATURE
          ========================================== */}
      <GlassCard>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
